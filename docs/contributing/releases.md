...
Milestones are the [Issue Tracker](https://github.com/foobar1-author/foobar1/issues){target=_blank} are used to track features to be included in upcoming releases.
...
4. Tag the release commit and push (typically this is the commit updating the
   version numbers).
   ```bash
   $ git tag -s v{VERSION} -m "foobar1 v{VERSION}"
